...
correlation = cor(data_merge2$log2FoldChange_BI_protac_24h,data_merge2$log2FoldChange_SMARCA4dtag_24h,method = "pearson",use="na.or.complete")

scatter <- ggplot(data_merge2, aes(x=log2FoldChange_BI_protac_24h, y=log2FoldChange_SMARCA4dtag_24h)) + scale_fill_gradientn(colours = cols,limits=c(0,1000),oob=squish) + geom_hex(bins = 70) + ylim(-9,9) + xlim(-9,9) + theme(legend.position = "none",panel.grid.major = element_blank(),panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) 

pdf(paste("scatterplot-all-regions-SMARCA4dtag24h-BI24h.pdf",sep=""))
...
merge_file <- merge(Novartis_24h,BI_protac_24h,"index")
...
correlation = cor(merge_file$log2FoldChange_N_24h,merge_file$log2FoldChange_BI_protac_24h,method = "pearson",use="na.or.complete")

scatter <- ggplot(merge_file, aes(x=log2FoldChange_BI_protac_24h, y=log2FoldChange_N_24h)) + xlim(-7.5,7.5) + ylim(-7.5,7.5) + scale_fill_gradientn(colours = cols,limits=c(0,1000),oob=squish) + geom_hex(bins = 70) + theme(legend.position = "none",panel.grid.major = element_blank(),panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) 

pdf(paste("scatterplot-all-regions-Novartis24h-BIprotac24h.pdf",sep=""))
print(scatter + annotate("text", x = -6, y = 6, label = c(paste("R = ",round(correlation,2),sep="")))) 
dev.off()
