...
setwd("/Users/sgrosche/Development/191127-BAF-timecourse-batch3/results/differential_analysis/191203-SMARCA4-timecourse-and-KO/")

### Reading in clusters FC based

cluster1 <- read.table("191204-cluster1-timecourse-only-regions-3-clusters-FC-based.txt",header = T)

cluster1$index <- row.names(cluster1)

cluster2 <- read.table("191204-cluster2-timecourse-only-regions-3-clusters-FC-based.txt",header = T)

cluster2$index <- row.names(cluster2)

cluster3 <- read.table("191204-cluster3-timecourse-only-regions-3-clusters-FC-based.txt",header = T)

cluster3$index <- row.names(cluster3)

### Reading in cqn counts

data_norm <- fread("/Users/sgrosche/Development/191127-BAF-timecourse-batch3/results/191127_BAF_Timecourse_Set3.matrix_cqn_norm_merged_replicates_191126_220139.tsv",data.table = F)
...
png("191205-Heatmap-FC-clustered-canberra-ward-rows-SMARCA4-timecourse-clustered-all-split3-FC-based-w-cqn-counts-next-split.png",height = 10000, width = 10000, res = 1200)
print(ht1+ht2+ht3)
dev.off()

pdf("200229-Heatmap-FC-clustered-canberra-ward-rows-SMARCA4-timecourse-clustered-all-split3-FC-based-w-cqn-counts-next-split.pdf")
