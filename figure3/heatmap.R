...
setwd("/Users/sgrosche/Development/191127-BAF-timecourse-batch3/results/differential_analysis/191210-all-timecourse/191210-SL-regions")

### reading in novel diff regions

ARID2KO <- read.table("191211-SL-ARID2KO-bg-SMARCA4-dtag-additional-differential-regions.bed",header = T)

ARID2KO$index <- paste(ARID2KO$chr,":",ARID2KO$pos1,"-",ARID2KO$pos2,sep = "")

BRGKO <- read.table("191211-SL-BRG-bg-BRM-dtag-additional-differential-regions.bed",header=T)

BRGKO$index <- paste(BRGKO$chr,":",BRGKO$pos1,"-",BRGKO$pos2,sep = "")

CC1KO <- read.table("191211-SL-CC1-bg-CC2-dtag-additional-differential-regions.bed",header = T)

CC1KO$index <- paste(CC1KO$chr,":",CC1KO$pos1,"-",CC1KO$pos2,sep = "")

diff_all <- rbind(ARID2KO,BRGKO,CC1KO)

cqn_counts <- read.table("/Users/sgrosche/Development/191127-BAF-timecourse-batch3/results/191127_BAF_Timecourse_Set3.matrix_cqn_norm_merged_replicates_191126_220139.tsv",header = T)
...
png("191212-Heatmap-Zscore-cqn-clustered-canberra-ward-rows-all-diff-regions-SL-additional.png",height = 10000, width = 10000, res = 1200)
print(ht1)
dev.off()
...
png("191216-Heatmap-Zscore-cqn-clustered-canberra-ward-rows-all-diff-regions-SL-additional-split5.png",height = 10000, width = 10000, res = 1200)
print(ht2)
dev.off()

pdf("200131-Heatmap-Zscore-cqn-clustered-canberra-ward-rows-all-diff-regions-SL-additional-split5.pdf")
print(ht2)
dev.off()
